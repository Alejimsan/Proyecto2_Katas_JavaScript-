...
for (const track of tracks){
    tracksGenre[track.genre].push(track.title);
}
console.log(tracksGenre)

/* VERSION si no sabemos los géneros, o en un futuro puede haber mas:

let tracksGenre = {};

for (const track of tracks) {
  if (!tracksGenre[track.genre]) {
    tracksGenre[track.genre] = [];
  }
  tracksGenre[track.genre].push(track.title);
}
